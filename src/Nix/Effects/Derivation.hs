...
getAttrOr' :: forall e f m v a. (MonadNix e f m, FromValue v m (NValue' f m (NValue f m)))
  => MS.HashMap Text (Free (NValue' f m) (Thunk m)) -> Text -> m a -> (v -> WithStringContextT m a) -> WithStringContextT m a
